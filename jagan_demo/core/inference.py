...
class Wav2LipInference:
    ...
    def load_wav2lip_openvino_model(self):
        ...
        print(devices[0])
        model_path = os.path.join(os.path.dirname(__file__), "openvino_model", "wav2lip_openvino_model.xml")
    ...
    def load_model(self):
        ...
        if self.device=='cpu':
            try:
                ...
                print(f"OpenVINO model not found or failed to load: {e}")
                print("Falling back to PyTorch on CPU...")
                return self.load_wav2lip_model(self.args.checkpoint_path)
        else:
            return self.load_wav2lip_model(self.args.checkpoint_path)

    def load_batch_face_model(self):
        model_path = os.path.join(os.path.dirname(__file__), "checkpoints", "mobilenet.pth")
        if self.device=='cpu':
